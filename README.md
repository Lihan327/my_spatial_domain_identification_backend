...
3. [性能评估](#3-性能评估)
4. [与 SOTA / 经典方法对比](#4-与-sota--经典方法对比)
5. [算法演进路线](#5-算法演进路线)
6. [成功经验与失败教训](#6-成功经验与失败教训)
...
> 本篇回答三个基础问题：什么是空间转录组与空间域识别？任务的意义与下游影响是什么？我们用的是什么数据？

---

## 1. 项目简介

### 1.1 空间域识别（Spatial Domain Identification）

**空间域** 是指在 **基因表达** 和 **组织形态学** 上具有相似性的连续区域。在空间转录组（ST）数据中，每个空间位点（spot）同时携带：

- **表达向量**（约 33,000 维基因表达）
- **空间坐标**（二维组织位置）

**空间域识别** 的目标是：对所有 spot 进行聚类，使同一空间域内的 spot 具有相似的表达模式与空间分布。
...
> 本篇回答三个核心问题：MAEST-X 在 DLPFC 12 切片上效果如何？与同类方法（SpaGCN、STAGATE、GraphST、CCST 等）相比有何优势？项目经历了哪些失败，又沉淀了哪些经验？

---

## 3. 性能评估

### 3.1 12 切片逐片结果（按 MAEST-X ARI 降序）
...
> 本篇深入剖析 MAEST-X 的技术细节：七大核心创新、整体流水线、7 种特征工程、6 种聚类方法、共识投票与边界保护机制。
...
> 本篇提供 API 索引、引用与致谢、许可证信息，供查阅与引用使用。
