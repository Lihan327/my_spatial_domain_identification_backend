"""
============================================================
空间域识别最终方案 (Final Spatial Domain Identification Pipeline)
============================================================

算法名称: Multi-Scale Spatial Clustering (MSSC)
作者: AI Training
日期: 2026
数据集: 人类背外侧前额叶皮层 (DLPFC) 12切片

============================================================
算法流程
============================================================
1. 数据预处理
   - 读取 10x Visium 格式 (h5 + 空间坐标)
   - 过滤 NA 标签的 spot
   - HVG 选取 (seurat_v3, top 3000 基因)
   - 标准化 (target_sum=1e4) + log1p

2. 多尺度空间平滑 (核心创新点)
   - 构建 6-NN 空间邻接图 (基于物理坐标)
   - 进行 3 种不同尺度的平滑:
     * (rounds=2, alpha=0.3) - 轻度平滑
     * (rounds=2, alpha=0.5) - 中度平滑
     * (rounds=3, alpha=0.7) - 强平滑
   - 拼接三个尺度的特征 (3 x 3000 = 9000 维)
...
4. 多 K 值 + 多种子 GMM 集成
   - 对 K in {5, 6, 7} 各尝试多个随机种子
   - 选取 silhouette + 调整 BIC 得分最高的聚类
   - covariance_type='full'

5. 后处理
   - 小簇清理 (占比 < 2% 的簇重指派)
   - 6-NN 空间众数投票 (2轮)
   - Hungarian 算法对齐预测标签与真值 (用于可视化)
...
- HS (Homogeneity Score): 同质性
- CS (Completeness Score): 完整性
- 计算每张切片的指标, 再求 12 张切片的均值与中位数
"""
import sys
sys.path.insert(0, '.')
import warnings
...
def spatial_smooth(X, knn_idx, rounds=2, alpha=0.5):
    ...
    Y = (1-alpha) * X + alpha * mean(X[neighbors])
    多轮迭代.
    """
    Y = X.copy()
...
def hungarian_remap(pred, gt):
    """Hungarian 算法: 把预测标签 ID 映射到最佳匹配的 GT 标签 ID."""
...
def process_slice(sid, data_root='DLPFC', out_root='DLPFC/DLPFC_result',
    ...
    print(f"  K={best_k} (true={n_layers}); ARI={ari:.4f} NMI={nmi:.4f} "
          f"HS={hs:.4f} CS={cs:.4f}  ({elapsed:.1f}s)")
...
def main():
    ...
    parser.add_argument('--csv_path', default='results/MSSC-Pipeline_per_slice_metrics.csv')
    ...
    for sid in SLICES:
        try:
            row = process_slice(sid, args.data_root, args.out_root,
                                n_seeds_gmm=args.n_seeds_gmm, pos_weight=args.pos_weight)
            rows.append(row)
