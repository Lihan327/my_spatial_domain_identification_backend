"""Pipeline v6: rank-based features + multi-seed + post-processing."""
import sys
sys.path.insert(0, '.')
import warnings
...
def process_slice(sid, data_root='DLPFC', out_root='DLPFC/DLPFC_result',
                  ...
                  K=7, n_seeds_gmm=5, post_iter=2, use_rank=False):
    print(f"\n========== {sid} ==========")
    t0 = time.time()
    ...
    # Optionally rank-transform
    if use_rank:
        from scipy.stats import rankdata
        X = np.apply_along_axis(rankdata, 0, X) / X.shape[0]  # percentile rank
    ...
    for it in range(post_iter):
        labels = small_cluster_cleanup(best_labels, knn_idx, min_ratio=0.02)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_path', default='results/HSGATE-v6_per_slice_metrics.csv')
    ...
    for sid in SLICES:
        try:
            row = process_slice(sid, args.data_root, args.out_root,
                                K=args.K, n_seeds_gmm=args.n_seeds_gmm,
