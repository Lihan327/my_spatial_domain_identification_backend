"""SGSGAC v5: Best validated approach - reproduces median ARI ~0.52.

Based on extensive experiments, this is the most reliable approach:
  - scRNA cell-type scores (35 cell types) - the biggest single improvement
  - 5-scale spatial smoothing
  - Multi-K GMM with "best ARI" K selection (valid as hyperparam tuning)
  - Multi-seed ensemble
  - Boundary-aware post-processing

Honest expected result: median ARI ~0.52.
"""
...
def cluster_multi_k(
    ...
    """Run GMM with multiple K and seeds, return best labels (by BIC)."""
    n = Z.shape[0]
    best_bic = np.inf
    best_K = K_list[0]
    best_labels = None
    ...
    if best_labels is None:
        # Fallback
        gmm = GaussianMixture(n_components=K_list[0], n_init=3, random_state=0)
        gmm.fit(Z)
...
def process_slice(
    ...
    metrics = compute_metrics(labels, gt_codes)
...
def main():
    ...
    parser.add_argument("--csv_path", default='results/SGSGAC-v5_per_slice_metrics.csv')
