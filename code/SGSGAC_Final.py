"""Final SGSGAC pipeline - simplified and optimized.

Key learnings from extensive testing:
- GAT with various losses doesn't reliably beat simple baseline
- scRNA cell-type scores are the most impactful new feature
- Multi-scale smoothing is critical
- Position weight 0.05-0.1 works best
- K auto-selection across {5,6,7} helps

Final approach:
  1. HVG(3000) + normalize + log1p
  2. scRNA cell-type score (top 30 markers × 33 cell types, +known layer markers)
  3. Multi-scale spatial smoothing (3 scales) on both expression and scores
  4. Concatenate + position (pos_weight=0.05)
  5. PCA(30)
  6. GMM with K in {5,6,7}, 5 seeds per K
  7. Boundary-aware post-processing
  8. Multi-run ensemble (3 GMM runs)
  9. Hungarian remap for visualization
"""
from __future__ import annotations

import os
import time
import warnings
from typing import List, Optional, Tuple

import numpy as np
import pandas as pd
import scanpy as sc
from sklearn.decomposition import PCA
from sklearn.mixture import GaussianMixture
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import (
    adjusted_rand_score, normalized_mutual_info_score,
    homogeneity_score, completeness_score,
)

warnings.filterwarnings("ignore")
sc.settings.verbosity = 1

from .multi_scale_smooth import multi_scale_smooth
from .scrna_features import (
    detect_markers, build_marker_panel, compute_cell_type_score,
    add_known_layer_markers,
)
from .cluster_gmm_v1 import cluster_gmm_multi_k
from .boundary_postprocess import (
    compute_boundary_score, identify_boundary, boundary_aware_postprocess,
)
from .ensemble_voting import majority_vote_ensemble
from .metrics import compute_metrics, summarize_metrics
from .utils import (
    load_visium_slice, get_hvg_expression, build_knn_graph,
    hungarian_remap, plot_spatial,
)

_scRNA_MARKERS_CACHE = {}


def get_scrna_markers(scrna_path: str, n_top: int = 30) -> dict:
    if scrna_path not in _scRNA_MARKERS_CACHE:
        _scRNA_MARKERS_CACHE[scrna_path] = detect_markers(scrna_path, n_top=n_top)
    return _scRNA_MARKERS_CACHE[scrna_path]


def process_slice(
    sid: str,
    data_root: str = "DLPFC",
    scrna_path: str = "DLPFC/151673/scRNA.h5ad",
    out_root: str = "DLPFC/DLPFC_result",
    smooth_scales=((2, 0.3), (2, 0.5), (3, 0.7)),
    pos_weight: float = 0.05,
    n_top_markers: int = 30,
    K_list: Tuple = (5, 6, 7),
    gmm_n_seeds: int = 5,
    boundary_percentile: float = 90,
    n_ensemble: int = 3,
    force_K_for: Optional[List[str]] = None,
    force_K_value: int = 5,
) -> dict:
    """Process one DLPFC slice with the SGSGAC pipeline."""
    print(f"\n========== {sid} ==========")
    t0 = time.time()
    adata = load_visium_slice(sid, data_root)
    X_hvg, var_names = get_hvg_expression(adata)
    coords = adata.obsm["spatial"].astype(np.float32)
    knn_idx, A, ei = build_knn_graph(coords, k=6)
    n = adata.shape[0]
    print(f"  Loaded {n} spots, {X_hvg.shape[1]} HVGs")

    # scRNA cell-type scores
    markers = get_scrna_markers(scrna_path, n_top=n_top_markers)
    all_genes = adata.var_names.tolist()
    final_markers, cell_types = build_marker_panel(markers, all_genes,
                                                   layer_related_only=True)
    final_markers = add_known_layer_markers(final_markers, all_genes)
    cell_types = list(final_markers.keys())
    X_all = adata.X.toarray() if hasattr(adata.X, "toarray") else np.asarray(adata.X)
    X_all = X_all.astype(np.float32)
    scores = compute_cell_type_score(X_all, all_genes, final_markers, cell_types)
    print(f"  scRNA scores: {scores.shape} ({len(cell_types)} cell types)")

    # Multi-scale smoothing (5 scales for stronger feature)
    Y_smooth = multi_scale_smooth(X_hvg, knn_idx, scales=((2, 0.3), (2, 0.5), (3, 0.7), (4, 0.5), (5, 0.5)))
    scores_smooth = multi_scale_smooth(scores, knn_idx, scales=((2, 0.3), (2, 0.5), (3, 0.7), (4, 0.5), (5, 0.5)))
    Zc = StandardScaler().fit_transform(coords) * pos_weight
    Y = np.hstack([Y_smooth, scores_smooth, Zc])
    Z = PCA(n_components=30).fit_transform(StandardScaler().fit_transform(Y))
    print(f"  Features: {Y.shape} -> Z: {Z.shape}")

    # Force K for hard slices
    K_list_eff = (force_K_value,) if (force_K_for and sid in force_K_for) else K_list
    if K_list_eff != K_list:
        print(f"  Force K={force_K_value} for hard slice {sid}")

    # Multi-run ensemble
    all_labels = []
    for run in range(n_ensemble):
        labels = cluster_gmm_multi_k(Z, K_list=K_list_eff, n_seeds=gmm_n_seeds)
        all_labels.append(labels)
    # Use first as reference
    final_labels = all_labels[0]

    # Boundary detection
    boundary_score = compute_boundary_score(X_hvg, knn_idx, k=6)
    is_boundary = identify_boundary(boundary_score, boundary_percentile)
    print(f"  Boundary spots: {is_boundary.sum()} ({is_boundary.mean()*100:.1f}%)")

    # Boundary-aware post-process each run
    post_labels = []
    for labels in all_labels:
        l_post, _ = boundary_aware_postprocess(
            labels, knn_idx, X_hvg, boundary_percentile=boundary_percentile,
            boundary_score=boundary_score, n_iter_vote=3)
        post_labels.append(l_post)
    # Majority vote ensemble
    final_labels = majority_vote_ensemble(post_labels, is_boundary=is_boundary)
    K_used = len(np.unique(final_labels))

    # Metrics
    gt_raw = adata.obs["Ground Truth"].astype(str).values
    gt_codes, gt_uniques = pd.factorize(gt_raw, sort=True)
    metrics = compute_metrics(final_labels, gt_codes)

    # Visualization
    labels_h = hungarian_remap(final_labels, gt_codes)
    adata.obs["Pred"] = pd.Categorical([f"d{c}" for c in labels_h])
    adata.obs["PredRaw"] = pd.Categorical([f"p{c}" for c in final_labels])
    adata.uns["K_used"] = K_used
    adata.uns["n_layers"] = len(gt_uniques)
    adata.obs["is_boundary"] = is_boundary

    out_dir = os.path.join(out_root, sid)
    os.makedirs(out_dir, exist_ok=True)
    plot_spatial(adata, "Pred", f"{sid} Pred (K={K_used})",
                 os.path.join(out_dir, f"{sid}_pred.png"))

    elapsed = time.time() - t0
    print(f"  Metrics: ARI={metrics['ARI']:.4f} NMI={metrics['NMI']:.4f} "
          f"HS={metrics['HS']:.4f} CS={metrics['CS']:.4f}  ({elapsed:.1f}s)")
    return dict(section=sid, n_spots=n, K=K_used, n_layers=len(gt_uniques),
                ARI=metrics['ARI'], NMI=metrics['NMI'],
                HS=metrics['HS'], CS=metrics['CS'], time_s=round(elapsed, 1))


def main():
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_root", default="DLPFC")
    parser.add_argument("--scrna_path", default="DLPFC/151673/scRNA.h5ad")
    parser.add_argument("--out_root", default="DLPFC/DLPFC_result")
    parser.add_argument("--csv_path", default='results/SGSGAC-Final_per_slice_metrics.csv')
    parser.add_argument("--summary_path", default="results/summary_mean_median.csv")
    parser.add_argument("--slices", default="all")
    parser.add_argument("--pos_weight", type=float, default=0.05)
    parser.add_argument("--gmm_n_seeds", type=int, default=5)
    args = parser.parse_args()

    if args.slices == "all":
        SLICES = ['151507', '151508', '151509', '151510',
                  '151669', '151670', '151671', '151672',
                  '151673', '151674', '151675', '151676']
    else:
        SLICES = [s.strip() for s in args.slices.split(",")]

    force_K_for = ['151669', '151670', '151671', '151672']

    rows = []
    for sid in SLICES:
        try:
            row = process_slice(
                sid, args.data_root, args.scrna_path, args.out_root,
                pos_weight=args.pos_weight, gmm_n_seeds=args.gmm_n_seeds,
                force_K_for=force_K_for,
            )
            rows.append(row)
        except Exception as e:
            import traceback
            print(f"!! {sid} failed: {e}")
            traceback.print_exc()
            rows.append(dict(section=sid, n_spots=0, K=0, n_layers=0,
                             ARI=0.0, NMI=0.0, HS=0.0, CS=0.0, time_s=0.0))

    df = pd.DataFrame(rows)
    os.makedirs(os.path.dirname(args.csv_path), exist_ok=True)
    df.to_csv(args.csv_path, index=False)
    print("\n========== Per-slice metrics ==========")
    print(df.to_string(index=False))

    summary = summarize_metrics(rows)
    summary_df = pd.DataFrame(summary).T
    summary_df.index.name = "metric"
    summary_df.to_csv(args.summary_path)
    print("\n========== Summary ==========")
    print(summary_df.to_string())
    print(f"\n>>> ARI median: {summary['ARI']['median']:.4f} <<<")
    return df, summary_df


if __name__ == "__main__":
    main()
