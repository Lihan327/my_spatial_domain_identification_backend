"""Final pipeline v3: best baseline + iterative refinement + K=7 forced."""
import sys
sys.path.insert(0, '.')
import warnings
...
def cluster_with_multi_seed(Z, K, n_seeds=10, return_sil=False):
...
def iterative_label_refinement(X, coords, K=7, n_iter=3):
    """Iteratively smooth labels, then re-cluster with refined features."""
    n = X.shape[0]
    nbrs = NearestNeighbors(n_neighbors=7, algorithm='ball_tree').fit(coords)
    _, knn_idx = nbrs.kneighbors(coords)
    knn_idx = knn_idx[:, 1:]

    Y = smooth_features(X, knn_idx, rounds=2, alpha=0.5)
    Z = PCA(n_components=30).fit_transform(StandardScaler().fit_transform(Y))
    labels = cluster_with_multi_seed(Z, K, n_seeds=10)

    for it in range(n_iter):
        # Pull each spot's features toward the mean of its cluster's neighbors
        Y_new = Y.copy()
        for i in range(n):
            same_cluster = (labels[knn_idx[i]] == labels[i])
            same_nbrs = knn_idx[i][same_cluster]
            if len(same_nbrs) > 0:
                # Average the spot's features with the average of same-cluster neighbors
                Y_new[i] = 0.5 * Y[i] + 0.5 * Y[same_nbrs].mean(axis=0)
        Y = Y_new
        # Light smoothing
        Y = smooth_features(Y, knn_idx, rounds=1, alpha=0.3)
        Z = PCA(n_components=30).fit_transform(StandardScaler().fit_transform(Y))
        labels = cluster_with_multi_seed(Z, K, n_seeds=10)
    return labels
...
def process_slice(sid, data_root='DLPFC', out_root='DLPFC/DLPFC_result',
                  K=7, smooth_rounds=2, smooth_alpha=0.5,
                  n_pca=30, n_iter_refine=2, n_seeds_gmm=10, post_iter=2):
    print(f"\n========== {sid} ==========")
    t0 = time.time()
    ...
    labels = iterative_label_refinement(X, coords, K=K, n_iter=n_iter_refine)
    ...
    plot_spatial(adata, 'Pred', f'{sid} Predicted (K={K}, true={n_layers})',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_path', default='results/HSGATE-v3_per_slice_metrics.csv')
    parser.add_argument('--summary_path', default='results/summary_mean_median.csv')
    parser.add_argument('--slices', default='all')
    parser.add_argument('--K', type=int, default=7)
    parser.add_argument('--n_iter_refine', type=int, default=2)
    parser.add_argument('--n_seeds_gmm', type=int, default=10)
    ...
    for sid in SLICES:
        try:
            row = process_slice(sid, args.data_root, args.out_root,
                                K=args.K, n_iter_refine=args.n_iter_refine,
