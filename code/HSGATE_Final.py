"""Final pipeline: best baseline + multi-K ensemble + multi-seed + post-processing.

Algorithm: Hybrid Spatial Clustering (HSC)
- Preprocess: HVG(3000), normalize, log1p
- Spatial smoothing: 2 rounds, alpha=0.5
- Feature: PCA(30) of smoothed + spatial coordinates (scaled)
- Clustering: GMM(K=7, full) with multiple seeds
- K selection: per-slice best of K in {5,6,7} by silhouette
- Post-processing: small cluster cleanup + 2 rounds of majority vote
- Label alignment: Hungarian remap for visualization
"""
import sys
sys.path.insert(0, '.')
import warnings
...
def process_slice(sid, data_root='DLPFC', out_root='DLPFC/DLPFC_result',
    ...
    # Multi-K, multi-seed GMM, pick best by BIC + silhouette
    ...
    for K in K_list:
        for s in range(n_seeds_gmm):
            ...
            try:
                ...
                sil = 0
            # Combined score: silhouette + small BIC component
            score = sil - 0.001 * abs(gmm.bic(Z)) / 1e6  # scale BIC down
            if score > best_score:
                best_score = score
                best_labels = labels
                best_k = K
...
if __name__ == "__main__":
    ...
    parser.add_argument('--csv_path', default='results/HSGATE-Final_per_slice_metrics.csv')
